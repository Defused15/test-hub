...
- **Multi-project aggregation** — combines Playwright (E2E) and Jest (unit) reports in one view
- **History tracking** — stores up to 30 run snapshots per project in `history/` (committed to git)
- **Charts** — pass rate trend, duration trend, flaky ranking, slowest tests, browser pass/fail comparison
- **Test detail panel** — click any row to slide in error message, stack trace, and retry history
- **Zero dependencies** — static site generated by a plain Node.js script; no database, no server
...
Tests run on push          push JSON   projects/
  → report.json    ──────────────────► project-name/
                   GitHub API (PUT)       latest.json
                                    ...
                                    updates history/<name>.json
                                    injects data into dashboard.html
                                    writes dist/ (index + project pages)
                                              │
                                       GitHub Pages deploy
...
│   └── your-other-project/
│       └── latest.json
├── history/                        # Auto-created by build; commit to persist trends
│   ├── QA-Playground-Tests.json    # Last 30 run snapshots
│   └── your-other-project.json
├── scripts/
│   ├── build.mjs                   # Entry point — orchestrates the full build
│   ├── lib/
│   │   ├── parsers.mjs             # detectReportType, parsePlaywright, parseJest
│   │   ├── history.mjs             # updateHistory (read/write history/*.json)
│   │   ├── render.mjs              # buildProjectPage (HTML generator)
│   │   └── utils.mjs               # stripAnsi, fmtDur
│   ├── dashboard.html              # Main dashboard template (data injected at build time)
...
> `dist/` is generated at build time and never committed — it's deployed directly to GitHub Pages by the workflow.
...
```
dist/
├── index.html              # Main dashboard (all projects aggregated)
├── style.css               # Copied from scripts/dashboard.css
├── projects/
│   ├── QA-Playground-Tests.html   # Per-project detail page
│   └── your-other-project.html
├── favicon.ico
├── favicon-16x16.png
├── favicon-32x32.png
├── apple-touch-icon.png
...
It also writes/updates `history/<project>.json` alongside the build (these are committed to git to persist trend data between deployments).
...
History files (`history/*.json`) are created automatically on the first build and updated on every subsequent run. Commit them to git so trend charts persist across deployments.
