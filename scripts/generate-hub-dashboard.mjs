...
writeFileSync(join(DIST, 'index.html'), buildHTML(DATA_JSON), 'utf8');
...
document.getElementById('cards').innerHTML = projects.map(p => {
  ...
  return '<div class="card">'
    ...
    + '<div><div class="card-name">'+p.name+'</div><div class="card-date">'+fmtDate(p.startTime)+'</div></div>'
    + '<span class="pill '+(p.stats.ok?'ok':'fail')+'"><span class="dot"></span>'+(p.stats.ok?'Passed':'Failed')+'</span>'
    + '</div>'
